...
//		AdPublic.addAd(BiliVideoViewActivity.this);
	...
	private void parseIntent(Intent i) {
		...
		// mUri = dat;
		// danmakuPath =
		// "http://comment.bilibili.com/"+i.getStringExtra("CID")+".xml";
	...
	private class VideoViewInitTask extends AsyncTask<String, Void, Integer> {
		...
		protected Integer doInBackground(String... arg0) {
			...
			Log.d("TAG", "开始解析视频地址");
			try {
				JSONObject videopathjson = new JSONObject(
						HttpUtil.getHtmlString("http://www.bilibili.com/m/html5?aid="
								+ av + "&page=" + page));
				Log.d("QAQ--->", "===>"
						+ videopathjson.getString("src").toString());
	...
	public boolean onKeyUp(int keyCode, KeyEvent event) {
		if (keyCode == KeyEvent.KEYCODE_BACK) {
			...
			if ((currentTime - touchTime) >= EXIT_INTERVAL_TIME) {
				Toast.makeText(BiliVideoViewActivity.this, "再按一下退出喵(｡･ω･｡)~", 1)
						.show();
